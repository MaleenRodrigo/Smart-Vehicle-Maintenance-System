...
router.post("/", async (req, res) => {
  try {
    const { invoiceId, customerName, billingAddress, issuedDate, contactNumber, items, subtotal, total} = req.body;
    ...
    if (err.code === 11000 && err.keyPattern && err.keyPattern.invoiceId === 1) {
    ...
    res
      .status(500)
      .json({ message: "Can't issue Invoice" });
  }
});
