...
const ReservationUpdate = () => {
  ...
  const res_ID = location.state?.reservationId; // Use optional chaining to avoid errors if state or res_Id is undefined
    ...
    const fetchReservation = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/reservation/res/${res_ID}`
        );
        // console.log(response.data);
        setResUpdate(response.data); // Set reservation data in the state
      } catch (error) {
